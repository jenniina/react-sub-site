...
export const useDragAndDrop = (initialState: Data[]) => {
  ...
  const [listItemsGood, setListItemsGood, removeListItemsGood] = useLocalStorage<Data[]>(
    'DnD-Good',
    goodItems
  )
  const [listItemsNeutral, setListItemsNeutral, removeListItemsNeutral] = useLocalStorage<
    Data[]
  >('DnD-Neutral', neutralItems)
  const [listItemsBad, setListItemsBad, removeListItemsBad] = useLocalStorage<Data[]>(
    'DnD-Bad',
    badItems
  )
      ...
      if (card && card.status !== status) {
        if (status === LOCATION.GOOD) {
          ...
          if (targetIndex == 0 || targetIndex == -1)
            setListItemsGood((prev) => [card!, ...prev])
          else if (listItemsGood && targetIndex == listItemsGood.length - 1)
            setListItemsGood((prev) => [...prev, card!])
          else
          ...
          if (targetIndex == 0 || targetIndex == -1)
            setListItemsBad((prev) => [card!, ...prev])
          else if (targetIndex == listItemsBad.length - 1) {
            setListItemsBad((prev) => [...prev, card!])
          } else
          ...
          if (targetIndex == 0 || targetIndex == -1)
            setListItemsNeutral((prev) => [card!, ...prev])
          else if (targetIndex == listItemsNeutral.length - 1)
            setListItemsNeutral((prev) => [...prev, card!])
          else
        ...
        if (targetCard.status === LOCATION.GOOD && cardGood) {
          ...
          if (targetIndex == 0)
            ...
            setListItemsGood((prev) => [
              ...
              cardGood!,
              ...prev
                .slice(targetIndex + 1, prev.length)
